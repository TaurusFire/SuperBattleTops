...
## appearing.
@export var lock_start_scale := 3.2
## Seconds the contraction takes.
@export var lock_time := 0.35
...
func _process(delta: float) -> void:
	if not _active or _target == null or _target.current_state == Top.State.STOPPED:
		release()
		return
